...
export default class CommitteePanel extends Component {
  render() {
    ...
    const {playerIndex, badFaction, currentMissionGroup, hasMadeSelection} = game;
    ...
    if(isPlayerOnMission && !hasMadeSelection) {
      return <div className="committee"> 
        <button
          className="action succeed"
          onClick={() => connection.sendMissionAction(true)}>
          Succeed
        </button>
        <button
          className="action fail"
          disabled={!badFaction}
          onClick={() => connection.sendMissionAction(false)}>
          Fail
        </button>
      </div>
    } else {
